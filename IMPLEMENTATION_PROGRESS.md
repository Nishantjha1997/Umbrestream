...
- Hero artwork quality fix is implemented but not yet pushed: the desktop hero and phone resume hero now promote TMDB `w500`/`w780`/`w1280`/`w1920` URLs to the `original` asset while leaving rails untouched; direct AniList/provider artwork passes through unchanged. Typecheck, focused lint, and `git diff --check` pass. The full StreamFree build was still running after the command wrapper timeout and was then found finished without a captured result; rerun a clean production build before or immediately after the next push.
