...
**Deployed and re-verified on production (2026-08-07).** Pushed as commit `e7d7853`; Vercel deployed it automatically. `/tv/97546/1/1/player` on `https://umbrestream.vercel.app` now defaults to `?src=vidking`, iframe has no `sandbox` attribute and correct `allow`/`referrerPolicy`, layout is full-bleed, and the Sources drawer lists all 8 providers in the new order. Full detail in `IMPLEMENTATION_PROGRESS.md` under "Live verification on production after the direct-mount rollback".

**One remaining gap:** actual cross-origin video playback inside the VidKing iframe (`readyState >= 2`, advancing `currentTime`) was not directly re-confirmed this session — this session's browser tooling cannot inspect a cross-origin frame's contents. The prior agent's verification recorded in `IMPLEMENTATION_PROGRESS.md` under "Live verification after TV rollback" already confirmed VidKing plays this exact fixture end-to-end when manually selected under the old engine; production now reaches that same provider/URL automatically. A manual watch-through, or browser automation capable of cross-origin frame inspection, would close this gap fully.

Also worth recording for future sessions: a stale PWA service worker on the production origin served the pre-rollback build for several minutes after the real deploy had gone live. If a fresh deploy looks like it hasn't shipped, check for and unregister a stale service worker before concluding the deploy failed.
