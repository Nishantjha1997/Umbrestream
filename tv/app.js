...
const LOCAL_LIBRARY_KEY = "streamfree-tv-library";
const LOCAL_HISTORY_KEY = "streamfree-tv-history";
const RECENT_SEARCH_KEY = "streamfree-tv-recent-searches";
...
function setPlaybackOrientation() {
  ...
  try {
    ...
    // Android TV's native bridge keeps the activity in landscape.
  }
}
...
function regionalParams(region, extra = {}) {
  return {
...
function tvFocusable(scope = document) {
  return [...scope.querySelectorAll(TV_FOCUS_SELECTOR)].filter((element) => {
...
function focusTvElement(element, { smooth = true } = {}) {
  if (!element) return false;
  element.focus({ preventScroll: true });
  element.scrollIntoView({
    behavior: smooth && !state.settings.reduceMotion ? "smooth" : "instant",
    block: "center",
    inline: "center",
  });
  return true;
...
function scheduleTvFocus(root = view) {
  window.setTimeout(() => {
    ...
    const preferred = root.querySelector(".hero .primary, .detail-content .primary, .page-head + * input, .guest-hero .primary, .profile-avatar, .player-back")
      || root.querySelector(TV_FOCUS_SELECTOR)
      || document.querySelector(".tab-bar a.active");
    focusTvElement(preferred, { smooth: false });
  }, 90);
}
...
async function renderHome() {
  ...
  try {
    ...
    const [trending, movies, shows, nowPlaying, topShows, topMovies, airing, regionalMovies, regionalSeries, bollywood, indianSeries, animeData] = await Promise.all([
    ...
    let picked = regionalMovies.results?.length ? regionalMovies.results : topMovies.results;
    ...
    commit(`${heroMarkup(hero, Boolean(history[0]))}
      ...
      ${section("Picked for you", picked.slice(0, 12), "movie", { kicker: state.user ? "Based on your watches" : "Critic favorites" })}
      ...
      ${section("Trending now", trending.results.slice(0, 14), "movie", { kicker: "Everyone is watching", ranked: true })}
      ${vibeMarkup([movies.results[2], movies.results[5], topMovies.results[2], shows.results[3]])}
      ...
      ${section("Fresh movie releases", nowPlaying.results.slice(0, 12), "movie", { kicker: "Now playing" })}
      ${section("Top-rated shows", topShows.results.slice(0, 12), "tv", { kicker: "All-time favorites" })}
      ${section("New episodes today", airing.results.slice(0, 12), "tv", { kicker: "Episode drops" })}
...
function closeSheet() {
  ...
  window.setTimeout(() => {
    sheetRoot.innerHTML = "";
    focusTvElement(focusBeforeSheet?.isConnected ? focusBeforeSheet : document.querySelector(".player-more"), { smooth: false });
...
function renderSettings() {
  commit(`<section class="subpage-head"><button class="back-fab pressable" data-action="back">‹</button><span class="eyebrow">Make it yours</span><h1>TV settings</h1><p>Stored privately on this Android TV device.</p></section><section class="settings-card">${settingToggle("dataSaver", "Data saver", "Prefer lighter artwork while browsing")}${settingToggle("autoplayNext", "Autoplay next episode", "Keep a series moving when supported")}${settingToggle("reduceMotion", "Reduce motion", "Use simpler screen transitions")}</section><section class="settings-note"><span>TV ad protection · Always on</span><p>Known advertising, tracking and popup hosts are blocked by the app WebView. Stream and video CDN requests remain allowed to protect playback.</p></section><section class="settings-note"><span>Native Android TV beta</span><p>The interface lives on your TV. Internet is used for account sync, title information, artwork and playback providers.</p></section>`, { root: "space" });
...
function renderHelp() {
  commit(`<section class="subpage-head"><button class="back-fab pressable" data-action="back">‹</button><span class="eyebrow">StreamFree TV</span><h1>Help & about</h1><p>A remote-first app powered by StreamFree's online catalogue and account services.</p></section><section class="faq-list"><details open><summary>How do I use the Mi TV remote?</summary><p>Use the arrows to move, OK to select, Back to return and Menu inside the player to choose another server.</p></details><details><summary>Can I use my website account?</summary><p>Yes. Sign in with the exact same email and password. Movies, series, library items and history use the same Supabase account.</p></details><details><summary>Does ad protection block every advert?</summary><p>The TV app blocks known advertising, tracking and popup hosts. Ads delivered from the same domain as a video can remain because blocking that domain would also stop playback.</p></details><details><summary>Why does a stream take time to start?</summary><p>Playback providers operate independently. Try another numbered server from the player if the first one is busy.</p></details><details><summary>Is this a website wrapper?</summary><p>No. The navigation, focus system, screens and animations are bundled in the TV APK. Only live data and streams are requested online.</p></details></section><section class="about-card"><div class="about-logo">SF</div><div><strong>StreamFree for Android TV</strong><span>Version 1.0 · Mi TV Remote Beta</span></div></section>`, { root: "space" });
}

function authShell(kind, body) {
  const copy = kind === "register" ? ["Join StreamFree", "One account for web, phone and TV."] : kind === "forgot" ? ["Reset password", "We will send a secure recovery link."] : ["Welcome back", "Continue with your StreamFree account."];
...
function focusDirection(current, direction) {
  const scope = sheetRoot.classList.contains("open") ? sheetRoot : document;
  const candidates = tvFocusable(scope).filter((element) => element !== current);
  if (!current || !current.getBoundingClientRect) return focusTvElement(candidates[0]);
  ...
  if (!winner && !sheetRoot.classList.contains("open")) {
    if (direction === "left") winner = document.querySelector(".tab-bar a.active");
    if (direction === "right" && current.closest(".tab-bar")) winner = view.querySelector(TV_FOCUS_SELECTOR);
  }
  return focusTvElement(winner);
}

document.addEventListener("focusin", (event) => {
  document.querySelectorAll(".tv-focus").forEach((element) => element.classList.remove("tv-focus"));
...
document.addEventListener("keydown", (event) => {
  ...
  if (directions[key] && !(editing && ["ArrowLeft", "ArrowRight"].includes(key))) {
    event.preventDefault();
    focusDirection(active, directions[key]);
    return;
  }
  if ((key === "Enter" || key === "NumpadEnter") && !editing && active?.matches(TV_FOCUS_SELECTOR)) {
    event.preventDefault();
    active.click();
    return;
  }
  if (key === "Escape" || key === "BrowserBack" || key === "GoBack") {
    if (remoteBack()) event.preventDefault();
    return;
  }
  if ((key === "ContextMenu" || event.keyCode === 82) && state.player) {
    event.preventDefault();
    showServerSheet();
    return;
  }
  if ((key === "MediaPlayPause" || event.keyCode === 85) && document.querySelector(".player-page")) {
    event.preventDefault();
    document.querySelector("[data-action='enter-player']")?.click();
  }
});
...
async function initializeNativeShell() {
  ...
  await NativeApp.addListener("backButton", () => {
    if (!remoteBack()) NativeApp.exitApp();
  });
